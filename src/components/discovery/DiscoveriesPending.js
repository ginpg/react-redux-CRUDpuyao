...
function DiscoveriesPending() {
  ...
  const pendingDiscoveryList = discoveryList.filter(discovery => discovery.status === 'pendiente')

  console.log(discoveryList)
  console.log(pendingDiscoveryList)

  // Fetch discoveries
  useEffect(() => {
    console.log(' ')
    console.log('Use Effect PENDING pidiendo discoveries')

    dispatch(getLastPage('pendientes'))
  ...
  const filteredDiscoveries = filterDiscoveries(pendingDiscoveryList, searchQuery)
  const action = '/gestion-operaciones/discoveries/pendientes/'
    ...
    <>
      <Navbar bg="light">
        <Nav variant="tabs" className="mr-auto">
          ...
          <Nav.Link onClick={go_pending_actioned} active>
            Discoveries Pendientes
          </Nav.Link>
...
export default DiscoveriesPending
